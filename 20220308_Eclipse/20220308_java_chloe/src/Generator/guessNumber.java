...
public class guessNumber extends JFrame{
	...
	private void doGuess() {
		...
		// must be digit
		try {
			enter = Integer.parseInt(usrInput);
		} catch(Exception e) {
			guess.setText("");
			JOptionPane.showMessageDialog(null, "Must be an integer", "ERROR", JOptionPane.ERROR_MESSAGE);
			count--;
		}
		...
		if(usrInput.matches(digit)) {
			...
			// guessing number cannot same as previous guessing
			if(hasDuplicates(enter) || enter < 1000 || enter > 9999) {
				guess.setText("");
	...
	private boolean hasDuplicates(int num) {
	...
	private void newGame() {
		...
		log.append("Please fill in " + NUM + "-digits\n");
